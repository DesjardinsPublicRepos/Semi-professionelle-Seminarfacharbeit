\documentclass[a4paper, 12pt]{scrartcl}

\usepackage[utf8]{inputenc}
\usepackage[english, ngerman]{babel}
\usepackage[T1]{fontenc}
\usepackage{amsmath}

%direkte pdf-links
\usepackage{hyperref}
%trassktript
\usepackage{xparse}
\usepackage{enumitem}
\setlist[description]{
  font={\sffamily\bfseries},
  labelsep=0pt,
  labelwidth=\transcriptlen,
  leftmargin=\transcriptlen,
}
\newlength{\transcriptlen}
\NewDocumentCommand {\setspeaker} { mo } {%
  \IfNoValueTF{#2}
  {\expandafter\newcommand\csname#1\endcsname{\item[#1:]}}%
  {\expandafter\newcommand\csname#1\endcsname{\item[#2:]}}%
  \IfNoValueTF{#2}
  {\settowidth{\transcriptlen}{#1}}%
  {\settowidth{\transcriptlen}{#2}}%
}

%für Bilder
\usepackage{graphicx}
%deutsche anführungszeichen
\usepackage{csquotes}\MakeOuterQuote{"}
%ränder
\usepackage[left=3cm, right=4cm, top=3cm, bottom=2.5cm]{geometry}
\usepackage[numbers, round]{natbib}
%spacing
\usepackage[onehalfspacing]{setspace}
%Abkürzungsverzeichnis
\usepackage[printonlyused, footnote]{acronym}
%Anzeigen der restlichen Verzeichnisse
\usepackage{tocbibind}
%Literaturverzeichnis
\bibliographystyle{alphadin}
%größere Schriftgrößen als 17pt
\usepackage{lmodern}
%bessere umbrüche
\usepackage{microtype}
%Bildquellen
\newcommand*{\bildquelle}{%
  \footnotesize Quelle:
}
%römische zahlen
\newcommand{\RM}[1]{\MakeUppercase{\romannumeral #1{.}}}
\makeatletter
\newenvironment{folding}{\endgroup}{\begingroup \def \@currenvir{folding}\edef \@currenvline{\on@line}}
\makeatother

%für H-Bild-option
\usepackage{float}

\hyphenation{Schleu-sing-en Haupt-ein-kaufs-mö-glich-keit Nie-drig-preis-po-li-tik}

\title{Seminarfacharbeit}
\author{Toni Hausdörfer, Fabian Beez}
\date{27. Mai 2020}

\begin{document}
    \input{exports/Deckblatt.tex}
    \input{exports/EidesstattlicheErklärung.tex}
    \input{exports/EidesstattlicheErklärung1.tex}
    \input{exports/Danksagung.tex}

    \tableofcontents 
        \newpage

    \section{Einleitung}
        \input{exports/Einleitung.tex}
        \newpage
    
    
    
    \section{Historie}
            \input{exports/Toni/historie/Einleitung.tex}
        \subsection{Entstehung}
            \input{exports/Toni/historie/1.tex}
        \subsection{Entwicklung}
            \input{exports/Toni/historie/2.tex}
        \subsection{Zusammenhang zwischen Onlinemarketing und -handel}
            \input{exports/Toni/historie/3.tex}
        \newpage
        
        
        
    \section{PayPal}
		\input{exports/Toni/paypal/Einleitung.tex}
        \subsection{Entstehung und grobe Funktionsweise}
            \input{exports/Toni/paypal/1.tex}
        \subsection{Negative Folgen}
            \input{exports/Toni/paypal/2.tex}
        \subsection{Einfluss auf die Gesellschaft}
            \input{exports/Toni/paypal/3.tex}
        \newpage
        
        
        
    \section{Konsumverhalten}
            \input{exports/Fabian/Konsum/Allgemein.tex}
        \subsection{Konsumverhalten und dessen Veränderung bezüglich der Vertriebsstrukturen}
            \input{exports/Fabian/Konsum/ÄnderungÜberZeit.tex}
            
        \subsection{Einschätzung und Folgen}
            \input{exports/Fabian/Konsum/ProblemeUndFolgen.tex}
        \newpage
            
            
            
    \section{Wirtschaft mit Blick auf Unternehmen}
            \input{exports/Toni/wirtschaft/einleitung.tex}
        \subsection{Entwicklung der Wirtschaft}
            \input{exports/Toni/wirtschaft/1.tex}
        \subsection{Auswirkungen auf Konzerne und Umwelt}
            \input{exports/Toni/wirtschaft/2.tex}
        \subsection{Auswirkungen auf Paketdienste}
            \input{exports/Toni/wirtschaft/3.tex}
        \newpage
        
        
        
    \section{Onlineversandhändler am Beispiel Amazon} 
        \input{exports/Fabian/Amazon/Allgemein.tex}
        \subsection{Entstehung und Entwicklung}
            \input{exports/Fabian/Amazon/Entwicklung.tex}
            
        \subsection{Einfluss auf das Konsumverhalten } %neu: das Konsumverhalten und der Einfluss des Onlinehandels
            \input{exports/Fabian/Amazon/EinflussAufKonsumverhalten.tex}
        \newpage
            
            
            
    \section{Globaler Vergleich des Einflusses und der Entwicklung des Onlinehandels}
            \input{exports/Fabian/GlobalerVergleich-EinflussUndEntwicklung.tex}
        \newpage
        
        
        
    \section{Ergebnisse}
        \subsection{Zusammenfassung}
            \input{exports/Fabian/konsum.tex}
            \input{exports/Toni/wirtschaft.tex}
            \input{exports/Zusammenfassung.tex}
        \subsection{Schlussfolgerungen im Bezug auf Schleusingen}
            \input{exports/ZuBeSchleusingen.tex}
        \newpage
        
        
        
    \bibliography{Literaturverzeichnis}
        \newpage
        
    \listoffigures
        \newpage
        
    \input{exports/Abkürzungsverzeichnis.tex}
    
    %\listoftables
    %    \newpage
    \addsec{Materialanhang}
        \newpage
    
    \input{exports/schuelerumfrage.tex}
    \input{exports/transskript.tex}
        \newpage
    \input{exports/einverstaendnis.tex}
        
        
\end{document}
